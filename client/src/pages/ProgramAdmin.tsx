...
type SheetRow = ProgramForm & { key: string; persisted: boolean };

const newKey = () => `program-row-${Date.now()}-${Math.random().toString(16).slice(2)}`;
const blankRow = (): SheetRow => ({ key: newKey(), persisted: false, externalId: "", title: "", summary: "", description: "", category: "", target: "", venue: "", startAt: "", endAt: "", recruitmentDeadline: "", recruitmentStatus: "upcoming", applicationUrl: "", applicationProvider: "none", contact: "", preApplicationChecks: "", imageUrl: "", isPublished: false });
...
export default function ProgramAdmin() {
  ...
  useEffect(() => {
    ...
    setRows(programsQuery.data.map(item => ({ ...item, key: item.externalId, persisted: true, startAt: toLocalInput(item.startAt), endAt: toLocalInput(item.endAt), recruitmentDeadline: toLocalInput(item.recruitmentDeadline) })));
  }, [programsQuery.data]);
  ...
  const uploadImage = async (key: string, event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      updateRow(key, "imageUrl", result.url);
      setNotice("사진을 올렸습니다. 해당 행의 저장 버튼을 눌러 프로그램에 연결하세요.");
  ...
  const saveRow = async (row: SheetRow) => {
    ...
    try {
      ...
      setRows(current => current.map(item => item.key === row.key ? { ...item, key: row.externalId.trim(), externalId: row.externalId.trim(), persisted: true } : item));
  ...
  if (!user) return <div className="program-admin-gate"><div><p className="reference-label"><span />관리자</p><h1>프로그램 관리</h1><p>프로그램을 시트처럼 직접 입력하려면 문화의집 프로젝트 소유자 계정으로 로그인해 주세요.</p><button onClick={() => startLogin()} className="reference-button">관리자 로그인</button></div></div>;
  if (!isAdmin) return <div className="program-admin-gate"><div><p className="reference-label"><span />접근 제한</p><h1>관리자 권한이 필요합니다.</h1><p>현재 계정에는 프로그램을 등록하거나 사진을 올릴 권한이 없습니다. 문화의집 프로젝트를 만든 계정으로 다시 로그인해 주세요.</p><Link href="/" className="reference-text-button">메인으로 돌아가기</Link></div></div>;

  return <main className="program-admin-page"><header className="program-admin-header"><Link href="/" className="reference-brand" aria-label="메인으로 이동"><span className="reference-brand__mark"><b>N</b></span><span><strong>남양주시 문화의집</strong><small>프로그램 시트</small></span></Link><Link href="/programs" className="reference-text-button">공개 화면 보기</Link></header><section className="program-sheet-shell"><div className="program-sheet-heading"><div><p className="reference-label"><span />관리자 입력</p><h1>프로그램 시트</h1><p>각 행에서 내용을 바꾸고 <b>저장</b>을 누르면 반영됩니다. 사진은 1080×1350px(4:5) 세로형으로 올려 주세요.</p></div><div className="program-sheet-heading__actions"><span>{rowCountLabel}</span><button type="button" className="reference-button" onClick={addRow}><Plus size={17} /> 새 행 추가</button></div></div>{programsQuery.isLoading && <p className="program-sheet-state"><Loader2 className="animate-spin" size={18} /> 프로그램 행을 불러오는 중입니다.</p>}{programsQuery.isError && <p className="program-sheet-state program-sheet-state--error">목록을 불러오지 못했습니다. 관리자 권한을 확인해 주세요.</p>}{!programsQuery.isLoading && <div className="program-sheet-scroll"><table className="program-sheet"><caption>프로그램 등록과 수정용 시트</caption><thead><tr><th>사진<br /><small>4:5</small></th><th>공개</th><th>프로그램 ID</th><th>프로그램 제목</th><th>한 줄 소개</th><th>주제</th><th>대상</th><th>장소</th><th>시작 일시</th><th>종료 일시</th><th>모집 마감</th><th>모집 상태</th><th>신청처</th><th>공식 신청 링크</th><th>문의처</th><th>상세 소개</th><th>신청 전 확인</th><th>저장</th><th>삭제</th></tr></thead><tbody>{rows.length === 0 && <tr><td colSpan={19} className="program-sheet-empty">등록한 프로그램이 없습니다. <button type="button" onClick={addRow}>첫 행 추가</button>로 시작하세요.</td></tr>}{rows.map(row => <tr key={row.key}><td><label className="program-sheet-image">{row.imageUrl ? <img src={row.imageUrl} alt={`${row.title || "프로그램"} 대표 사진`} /> : <span><ImagePlus size={20} />사진</span>}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => uploadImage(row.key, event)} disabled={uploadMutation.isPending} /></label></td><td><select aria-label={`${row.title || "새 프로그램"} 공개 상태`} value={row.isPublished ? "published" : "draft"} onChange={event => updateRow(row.key, "isPublished", event.target.value === "published")}><option value="draft">비공개</option><option value="published">공개</option></select></td><td><input aria-label="프로그램 ID" value={row.externalId} disabled={row.persisted} onChange={event => updateRow(row.key, "externalId", event.target.value.toLowerCase())} placeholder="2026-autumn" /></td><td><input aria-label="프로그램 제목" value={row.title} onChange={event => updateRow(row.key, "title", event.target.value)} placeholder="프로그램 제목" /></td><td><input aria-label="한 줄 소개" value={row.summary} onChange={event => updateRow(row.key, "summary", event.target.value)} placeholder="한 줄 소개" /></td><td><input aria-label="주제" value={row.category} onChange={event => updateRow(row.key, "category", event.target.value)} placeholder="문화예술" /></td><td><input aria-label="대상" value={row.target} onChange={event => updateRow(row.key, "target", event.target.value)} placeholder="성인" /></td><td><input aria-label="장소" value={row.venue} onChange={event => updateRow(row.key, "venue", event.target.value)} placeholder="문화의집" /></td><td><input aria-label="시작 일시" type="datetime-local" value={row.startAt} onChange={event => updateRow(row.key, "startAt", event.target.value)} /></td><td><input aria-label="종료 일시" type="datetime-local" value={row.endAt} onChange={event => updateRow(row.key, "endAt", event.target.value)} /></td><td><input aria-label="모집 마감일" type="datetime-local" value={row.recruitmentDeadline} onChange={event => updateRow(row.key, "recruitmentDeadline", event.target.value)} /></td><td><select aria-label="모집 상태" value={row.recruitmentStatus} onChange={event => updateRow(row.key, "recruitmentStatus", event.target.value as ProgramForm["recruitmentStatus"])}><option value="upcoming">예정</option><option value="open">모집 중</option><option value="closing-soon">마감 임박</option><option value="closed">마감</option></select></td><td><select aria-label="신청처" value={row.applicationProvider} onChange={event => updateRow(row.key, "applicationProvider", event.target.value as ProgramForm["applicationProvider"])}><option value="none">준비 중</option><option value="nyjcf">문화재단</option><option value="naver">네이버폼</option><option value="other">기타</option></select></td><td><input aria-label="공식 신청 링크" type="url" value={row.applicationUrl} onChange={event => updateRow(row.key, "applicationUrl", event.target.value)} placeholder="https://" /></td><td><input aria-label="문의처" value={row.contact} onChange={event => updateRow(row.key, "contact", event.target.value)} placeholder="031-000-0000" /></td><td><textarea aria-label="상세 소개" value={row.description} onChange={event => updateRow(row.key, "description", event.target.value)} placeholder="상세 소개" /></td><td><textarea aria-label="신청 전 확인" value={row.preApplicationChecks} onChange={event => updateRow(row.key, "preApplicationChecks", event.target.value)} placeholder="한 줄에 한 항목" /></td><td><button type="button" className="program-sheet-save" onClick={() => saveRow(row)} disabled={saveMutation.isPending || uploadMutation.isPending}><Save size={16} /> 저장</button></td><td><button type="button" className="program-sheet-delete" onClick={() => removeRow(row)} disabled={deleteMutation.isPending}><Trash2 size={16} /><span className="sr-only">삭제</span></button></td></tr>)}</tbody></table></div>}{notice && <p className="program-sheet-notice" role="status">{notice}</p>}{error && <p className="program-sheet-notice program-sheet-notice--error" role="alert">{error}</p>}</section></main>;
}
